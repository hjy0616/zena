...
func TestOutOfTurnSigning(t *testing.T) {
	...
	for i := uint64(1); i < spanSize; i++ {
		block = buildNextBlock(t, _zena, chain, block, nil, init.genesis.Config.Zena, nil, heimdallSpan.ValidatorSet.Validators, setDifficulty)
		insertNewBlock(t, chain, block)
	}
	...
	block = buildNextBlock(t, _zena, chain, block, signerKey, init.genesis.Config.Zena, nil, heimdallSpan.ValidatorSet.Validators, setParentTime, setDifficulty)
...
func TestSignerNotFound(t *testing.T) {
	...
	block = buildNextBlock(t, _zena, chain, block, signerKey, init.genesis.Config.Zena, nil, heimdallSpan.ValidatorSet.Validators)
