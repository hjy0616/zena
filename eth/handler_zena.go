...
func (h *ethHandler) fetchWhitelistCheckpoint(ctx context.Context, zena *eirene.Zena, eth *Zenanet, verifier *zenaVerifier) (uint64, common.Hash, error) {
	var (
		blockNum  uint64
	...
	checkpoint, err := zena.HeimdallClient.FetchCheckpoint(ctx, -1)
...
// and verifies the data against zena data.
func (h *ethHandler) fetchWhitelistMilestone(ctx context.Context, zena *eirene.Zena, eth *Zenanet, verifier *zenaVerifier) (uint64, common.Hash, error) {
	var (
		num  uint64
		hash common.Hash
	)
	...
	milestone, err := zena.HeimdallClient.FetchMilestone(ctx)
...
func (h *ethHandler) fetchNoAckMilestone(ctx context.Context, zena *eirene.Zena) (string, error) {
	milestoneID, err := zena.HeimdallClient.FetchLastNoAckMilestone(ctx)
	err = reportCommonErrors("latest no-ack milestone", err, nil)

	return milestoneID, err
}

func (h *ethHandler) fetchNoAckMilestoneByID(ctx context.Context, zena *eirene.Zena, milestoneID string) error {
	err := zena.HeimdallClient.FetchNoAckMilestone(ctx, milestoneID)
